  ...
  return (
    <>
      {posts.map((post) => (
        <div
          key={post._id}
          className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow mb-4 flex"
        >
          <div className="w-1/4 flex flex-col items-center">
            <img
              src={require(`../../assets/${post.user.profilePicture}`)}
              alt={post.user.name}
            ...
            <p className="mt-2 text-gray-600 dark:text-gray-400">
              ...
              {post.likes && post.likes.length === 1 ? 'person likes' : 'people like'} this post
            </p>
          </div>
